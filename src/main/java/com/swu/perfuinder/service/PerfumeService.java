...
public class PerfumeService {
    ...
    public HomeResponse.HomeInfo getHomeInfo() {
        ...
        List<PerfumeResponse.RandomSeasonPerfumeInfo> seasonPerfumes =
                perfumeRepository.findRandomBySeason(String.valueOf(randomSeason), 5)
                        .stream()
                        .map(perfumeConverter::toRandomSeasonPerfumeInfo)
                        .collect(Collectors.toList());

        List<PerfumeResponse.RandomBrandPerfumeInfo> brandPerfumes =
                perfumeRepository.findRandomByBrand(String.valueOf(randomBrand), 5)
                        .stream()
